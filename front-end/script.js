...
document.addEventListener('DOMContentLoaded', () => {
    ...
    btnBuscar.addEventListener('click', async () => {
        const nomeParaBuscar = inputNomePesquisadora.value.trim(); // Pega o valor do input
        ...
        try {
            ...
            const backendUrl = 'http://localhost:3000'; // OU a URL real do seu backend
            ...
            if (data.length > 0) {
                ...
                data.forEach(pesquisadora => {
                    ...
                    const lattesLink = pesquisadora.link_lattes
                        ? `<a href="${pesquisadora.link_lattes}" target="_blank">Ver Lattes</a>`
                        : 'Não disponível';
                    htmlResultados += `<td>${pesquisadora.link_lattes}</td>`;
